...
pub(super) struct NodeConstraintBindPlugin;
...
impl Plugin for NodeConstraintBindPlugin {
    fn build(
        ...
        app.add_systems(Update, (bind_rotation_constraints,));
    }
}

fn bind_rotation_constraints(
    par_commands: ParallelCommands,
    sources: Query<(&Transform, &RestTransform, &RotationConstraintDestinations), Changed<Transform>>,
    dests: Query<(&Transform, &RestTransform)>,
) {
    sources.par_iter().for_each(|(src_tf, src_rest, destinations)| {
        for dest in &destinations.0 {
            if let Ok((dest_tf, dest_rest)) = dests.get(dest.dest) {
                ...
                let new_rot = dest_rest.rotation.slerp(dest_rest.rotation * src_delta_quat, dest.weight);
